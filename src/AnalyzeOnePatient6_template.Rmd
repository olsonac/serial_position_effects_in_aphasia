...
ggsave(paste0(FigDir,CurPat,"_",RunLabel,"_",CurTask,"_pos_of_syll_complexities_in_stimuli.png"),plot=syll_comp_plot,device="png",unit="cm",width=15,height=11)
syll_comp_plot
```
...
write.csv(pos_len_table,paste0(TablesDir,CurPat,"_",RunLabel,"_",CurTask,"_preserved_percentages_by_len_position.csv"))
pos_len_table
```
...
LPAICSummary <- LPAICSummary %>% arrange(AIC)
...
ggsave(paste0(FigDir,CurPat,"_",RunLabel,"_",CurTask,"_percent_preserved_by_length_pos_wfit.png"),plot=fitted_len_pos_plot,device="png",unit="cm",width=15,height=11)
fitted_len_pos_plot
```
...
NoFragLPAICSummary <- NoFragLPAICSummary %>% arrange(AIC) # insure order by AIC
...
HF_Plot <- plot_len_pos_obs_predicted(HFDat,paste0(CurPat," - ",RunLabel," - High frequency"),"FLPFitted",c(min_preserved,max_preserved),palette_values,shape_values, obs_linetypes,NULL)
LF_Plot <- plot_len_pos_obs_predicted(LFDat,paste0(CurPat," - ",RunLabel," - Low frequency"),"FLPFitted",c(min_preserved,max_preserved),palette_values,shape_values,obs_linetypes,NULL)
library(ggpubr)
...
MEAICSummary <- MEAICSummary %>% arrange(AIC)
...
CumAICSummary$DeltaAIC <- CumAICSummary$AIC - CumAICSummary$AIC[[1]]
